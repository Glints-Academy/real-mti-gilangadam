const whatTimeIsIt = (time) => {
    ...
    if(index === "pm") {
      if(minutes > 19) {
        return `It's ${smallNum[hours]} past ${bigNum[dozen]} ${smallNum[unit]} in the afternoon`
      } else {
        return `It's ${smallNum[hours]} past ${smallNum[minutes]} in the afternoon`
      }
    } else {
      if(minutes > 19) {
        return `It's ${smallNum[hours]} past ${bigNum[dozen]} ${smallNum[unit]} in the morning`
      } else {
